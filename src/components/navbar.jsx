...
const Navbar = ({ onClose }) => {
  ...
  const links = [
    { name: "Linkedin", url: "https://www.linkedin.com/in/kesavanperumalswamy/" },
  ...
  return (
    <div className="relative">
      ...
      <div
        ref={navbarRef}
        className="bg-black h-screen overflow-hidden flex flex-col justify-between relative z-20"
      >
        <div className="flex justify-between px-4 items-center pt-5 ">
          <div className="group font-walbaum relative flex justify-center overflow-hidden cursor-pointer text-2xl leading-6">
            <NavLink
              onClick={handleClose}
              to="/"
              className={({ isActive }) => 
                `inline-block relative p-1 transition duration-500 text-white ease-out group-hover:-translate-y-[120%] ${isActive ? 'text-[#FF7F3E]' : ''}`
              }
            >
              K7's Portfolio
            </NavLink>
          ...
          <div className="md:pr-32">
            <a onMouseOver={handleMouseover} onMouseOut={handleMouseOut} target="_blank" href="mailto:contact@kesavanperumalsamy.in" className="font-cerotta tracking-widest text-white">Email me</a>
            </div>
  ...
  <div className="grid gap-4 text-center">
    {[
      ...
      <div className="relative group" key={name} ref={(el) => (menuItemsRef.current[index] = el)}>
        <NavLink
          to={path}
          className={({ isActive }) =>
            `cursor-pointer menu-item text-${isActive ? 'Secondary' : 'white'} relative md:text-[80px] text-[50px] will-change-transform hover:tracking-[0.075rem] font-cerotta`
          }
        >
          {name}
        </NavLink>
...
<div className="flex font-altone-oblique z-50 space-x-2 px-20 items-center justify-between py-4">
